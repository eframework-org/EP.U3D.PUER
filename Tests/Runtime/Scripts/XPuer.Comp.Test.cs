...
public class TestXPuerComp
{
    ...
    public void Setup()
    {
        testModule = XPuer.VM.ExecuteModule("TestComponent").Get<JSObject>("MyComponent");
    }
    ...
    public void Property(bool isDynamic)
    {
        ...
        {
            obj = new GameObject();
            PuerBehaviour.Add(obj, "", testModule);
            puerComp = obj.GetComponent<PuerBehaviour>();
        }
        else
        {
            obj = GameObject.Instantiate(Resources.Load<GameObject>("Prefab/AddAndGet"));
            puerComp = obj.GetComponent<PuerBehaviour>();
            Source.Parse();
    ...
    {
        LogAssert.Expect(LogType.Log, "TestComponent Awake");
        LogAssert.Expect(LogType.Log, "TestComponent OnEnable");
        LogAssert.Expect(LogType.Log, "TestComponent Start");
        LogAssert.Expect(LogType.Log, "TestComponent Update");
        LogAssert.Expect(LogType.Log, "TestComponent LateUpdate");
        LogAssert.Expect(LogType.Log, "TestComponent FixedUpdate");
        LogAssert.Expect(LogType.Log, "TestComponent OnDisable");
        LogAssert.Expect(LogType.Log, "TestComponent OnDestroy");
    ...
    {
        LogAssert.Expect(LogType.Log, "TestComponent OnCollisionEnter");
        LogAssert.Expect(LogType.Log, "TestComponent OnCollisionStay");
        LogAssert.Expect(LogType.Log, "TestComponent OnCollisionExit");
        LogAssert.Expect(LogType.Log, "TestComponent OnTriggerEnter");
        LogAssert.Expect(LogType.Log, "TestComponent OnTriggerStay");
        LogAssert.Expect(LogType.Log, "TestComponent OnTriggerExit");
        ...
        PuerBehaviour.Add(cube1, "", testModule);
    ...
    public void Get(int depth, int rootResult, int child1Result, int grandChildResult, int child2Result, string _)
    {
        ...
        try
        {
            ...
            // 添加测试组件
            PuerBehaviour.Add(root, "", testModule);
            PuerBehaviour.Add(child1, "", testModule);
            PuerBehaviour.Add(child2, "", testModule);
            PuerBehaviour.Add(grandChild, "", testModule);

            // 在不同节点上测试指定深度
            PuerBehaviour[] result;

            // 测试Root
            result = PuerBehaviour.InternalGets(root, testModule, depth, true);
            Assert.AreEqual(rootResult, result.Length);

            // 测试Child1
            result = PuerBehaviour.InternalGets(child1, testModule, depth, true);
            Assert.AreEqual(child1Result, result.Length);

            // 测试GrandChild
            result = PuerBehaviour.InternalGets(grandChild, testModule, depth, true);
            Assert.AreEqual(grandChildResult, result.Length);

            // 测试Child2
            result = PuerBehaviour.InternalGets(child2, testModule, depth, true);
            ...
            result = PuerBehaviour.InternalGets(root, testModule, depth, false);
            ...
            result = PuerBehaviour.InternalGets(null, testModule, depth, true);
